            ...
            <div class="bg-white dark:bg-gray-900 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    ...
                    <table id="agencyTable" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @if ($agencies->where('user_id', auth()->user()->id)->count() > 0)
                                @foreach ($agencies as $agency)
                                    @if ($agency->user_id == auth()->user()->id)
                                        <tr class="border-b dark:border-gray-700" id="{{ 'agency_' . $agency->id }}">
                                            <td>
                                                <img class="w-10 h-10 p-1 rounded-full ring-2 ring-secondary dark:ring-primary"
                                                    src="{{ asset('storage/' . $agency->media->first()->file_path) }}"
                                                    alt="Agency Image" class="h-8 w-auto mr-3">
                                            </td>
                                            <td
